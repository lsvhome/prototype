...
#if DEBUG
            var ddd = @"C:\Users\Sergii Lutai\AppData\Local\FEX.NET";
            var xxx = new System.IO.DirectoryInfo(ddd);
            if (xxx.Exists)
            {
                xxx.Delete(true);
            }

            string[] hhh = new[] { @"C:\Temp\666", @"C:\Temp\777", @"C:\Temp\888" };
            foreach (var each in hhh)
            {
                var ttt = new System.IO.DirectoryInfo(each);
                if (ttt.Exists)
                {
                    ttt.Delete(true);
                }
