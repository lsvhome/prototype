...
namespace FexSync.Data
{
    ...
    public class Log4NetTraceListener : TraceListener
    {
        ...
        {
            this.WriteLine((object)message, category);
        }
        ...
        private static StackFrame GetTracingStackFrame(StackTrace stack)
        {
            for (var i = 0; i < stack.FrameCount; i++)
            {
                ...
                if (method.DeclaringType.Namespace == "System.Diagnostics")
                {
                    // ReSharper restore PossibleNullReferenceException
                    continue;
                }

                if (method.DeclaringType.Namespace == "System.Threading")
                {
                    continue;
                }

                if (method.DeclaringType.Name == "Log4NetTraceListener")
                {
                    continue;
                }
