...
#endif
                        var remoteFile = this.SyncDb.RemoteFiles.SingleOrDefault(item => item.Path == ui.Path);
                        var removeRange = this.SyncDb.RemoteFiles
                            .Where(item => item.Path == ui.Path && item != remoteFile)
                            .ToArray();

                        this.SyncDb.RemoteFiles.RemoveRange(removeRange);
                        if (remoteFile == null)
                        {
